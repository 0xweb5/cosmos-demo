...
function Keplr() {
	...
	const [recipent, setRecipent] = useState<any>(
		"osmo1xldvc06ha3zefkl2tlxhj6jpwgxny2ft0pd2nr"
	);
	...
	const getBalances = async () => {
		const balance = await client?.getBalance(address, chain.feeCurrencies[0].coinMinimalDenom);
		setBalance(balance);
	};

	// txhash查询  Todo
	const getTx = async () => {
		const txRes = await client?.getTx(tx);
		setTxRes(txRes);
	};
	...
	const sendToken = async () => {
		...
		try {
			...
			if (result.code === 0) {
				alert(`transfer success, height: ${result.height}, txhash: ${result.transactionHash}`);
